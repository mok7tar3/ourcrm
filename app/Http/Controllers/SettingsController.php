    ...
    public function store(Request $request)
    {
        ...
        if($user->can('manage-setting'))
        {
            if($request->favicon)
            {
                ...
                $path = Utility::upload_file($request,'favicon',$favicon,$fullPath,$validation);
                if($path['flag'] == 1){
                    // $favicon = $path['url'];
                    $post['favicon'] = $dir . $favicon;
                }else{
            ...
            if(!empty($request->logo))
            {
                ...
                $path = Utility::upload_file($request,'logo',$logoName,$fullPath,$validation);
                if($path['flag'] == 1){
                    // $logo = $path['url'];
                    $post['logo'] = $dir . $logoName;
                }else{
                    return redirect()->back()->with('error', __($path['msg']));
                }
            ...
            if($request->white_logo)
            {
                ...
                $path = Utility::upload_file($request,'white_logo',$lightlogoName,$fullPath,$validation);

                if($path['flag'] == 1){
                    // $white_logo = $path['url'];
                    $post['white_logo'] = $dir . $lightlogoName;
                }else{
                    return redirect()->back()->with('error', __($path['msg']));
                }
            }
    ...
    public static function setEnvironmentValue(array $values)
    {
        ...
        foreach ($values as $envKey => $envValue) {
            ...
            if (preg_match($keyPattern, $str)) {
                ...
                $str = preg_replace($keyPattern, $newLine, $str);
            } else {
                // Append new line
                $str .= PHP_EOL . $newLine;
            }
        ...
        return file_put_contents($envFile, trim($str) . PHP_EOL) !== false;
    }
    ...
    public function saveSEOSettings(Request $request)
    {
        $validator = \Validator::make(
            ...
            [
                ...
                'meta_image' => 'nullable|mimes:png,jpg,jpeg,gif',
            ]
        );
        if ($validator->fails()) {
            $messages = $validator->getMessageBag();
        ...
        if ($request->meta_image) {
            ...
            $path = Utility::upload_file($request,'meta_image',$img_name,$fullPath,$validation);
