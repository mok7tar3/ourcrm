...
export default function OurMissionSection() {
  ...
  // Load content from CMS with unique key 'about_mission'
  const titleAr = useContentValue('about_mission', 'titleAr', 'مهمتنا');
  const titleEn = useContentValue('about_mission', 'titleEn', 'Our Mission');
  const textAr = useContentValue('about_mission', 'textAr', 'تقديم تجربة دعم فعالة وسهلة الوصول تمكن أعضاء جامعة الملك سعود وتعزز ثقافة الشفافية والاستجابة والتحسين المستمر.');
  const textEn = useContentValue('about_mission', 'textEn', 'To deliver an efficient and accessible support experience that empowers KSU members and promotes a culture of transparency, responsiveness, and continuous improvement.');

  return (
    <div className="relative w-full" data-name="Our_Mission_section">
